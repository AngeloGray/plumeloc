...
def main():
    # Переменная для файлов логов / дебаггинга
    ...
    logs_file.write(f"- - - - - - - \nLOGS FILE\n- - - - - - - \n"
                    f"Date and time: {time.asctime()}\n"
                    f"Configuration is:\n"
                    f"TERRITORY_SIZE = {TERRITORY_SIZE}\n"
                    f"MASHTAB_COEF = {MASHTAB_COEF}\n"
                    f"WIND_DIRECTION = {WIND_DIRECTION}\n"
                    f"PLUME_SIZE = {PLUME_SIZE}\n\n"
                    f"[OPTIONAL] CUSTOM COORDS: {PLUME_CUSTOM_COORDS}\n"
                    f"Number of UAV's -> {number_of_uavs}\n")
    ...
    for i in range(number_of_uavs):
        world_uav.append(World(logs_file=logs_file))
        world_uav[i].world_create_uav()
        uav_id = i
        uav.append(UAV(
            id=uav_id,
    ...
    world_global = World(logs_file=logs_file)
    world_global.world_create()  # Создание глобальной карты
    mpl_paint_weights_map(uav, 0, world_global)  # Прорисовка изначальной карты весов
    ...
    # Начало цикла, осуществляющего последовательный параллельный алгоритм для каждого дрона
    mission_is_active: bool = True  # флаг конца миссии - в данном случае вызывается при обнаружении источника
    uav_found_plume: int = -1  # флаг для определения id первого нашедшего загрязнение дрона
    time_stamp: int = 0  # переменная определяющая итерации. Каждая итерация начинается и заканчивается одним действием
    ...
    while mission_is_active:
        ...
        for n in range(number_of_uavs):
            if uav[n].cur_mode == 'SEARCH':  # Режим поиска каких-либо следова загрязнения
                print(f"uav{n} getting target point in SEARCH mode")
                logs_file.write(f"uav{n} getting target point in SEARCH mode\n")
                uav[n].get_target_point_searching()
            elif uav[n].cur_mode == 'LOCALIZE':  # Режим локализации - след найден, необходимо найти источник
                print(f"uav{n} getting target point in LOCALIZE mode")
                logs_file.write(f"uav{n} getting target point in LOCALIZE mode\n")
                uav[n].get_target_point_localization()
            elif uav[n].cur_mode == 'REACH':  # Режим следования, в данном случае за нашедшим загрязнение дроном
                print(f"uav{n} getting target point in REACH mode")
                logs_file.write(f"uav{n} getting target point in REACH mode\n")
        ...
        # данная секция влияет на ситуации, когда дроны претендуют на одни и те же точки
        for i in range(number_of_uavs):
            for j in range(number_of_uavs):
                if i != j:
                    if uav[i].target_point.id == uav[j].target_point.id:
        ...
        for n in range(number_of_uavs):
            ...
            uav[n].move_to(uav[n].target_point)
            logs_file.write(f"uav with id {n} moved to target point: ({uav[n].cur_point.x}, {uav[n].cur_point.y})\n")

        for n in range(number_of_uavs):
            # 2) Измерение уровня концентрации ЗВ в точке, на который переместился дрон
            uav[n].measure_plume(world_global.points[(int(uav[n].cur_point.x), int(uav[n].cur_point.y))])
            logs_file.write(f"uav with id {n} measured c = {uav[n].cur_point.c} in "
                            f"Point ({uav[n].cur_point.x},{uav[n].cur_point.y})\n")
